...
const EditPUC = ({ details, setRealtime, setOpenVehicleDocumentDrawer }) => {
  console.log(details);
  const [value, setValue] = useState(details?.pollution?.url);
  const initialValues = {
    pucNumber: "",
    validTill: "",
  };
  ...
  const handleEditPuc = async (values, submitProps) => {
    console.log(values);
    const formdata = new FormData();
    value !== details?.pollution?.url &&
    ...
    try {
      ...
      console.log(res);
      res?.status === 200
        ? Swal.fire("Success", "Pollution Info Updated", "success")
        : Swal.fire("Error", "Pollution Info Not Updated", "error");
    ...
    <>
      ...
      <Formik
        initialValues={
          details?.pollution
            ? {
                pucNumber: details?.pollution?.number,
                validTill:
                  details?.pollution?.expiry &&
                  new Date(details?.pollution?.expiry)
                    .toISOString()
                    .split("T")[0],
              }
            : initialValues
