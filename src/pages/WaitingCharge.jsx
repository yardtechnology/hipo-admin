...
import { WaitingChargeSchema } from "schemas";
...
const WaitingCharge = () => {
  ...
  const validationSchema = WaitingChargeSchema.reduce(
    (accumulator, currentValue) => {
  ...
  const handleSetProfit = async (values, submitProps) => {
    ...
    try {
      ...
      result.status === 200
        ? Swal.fire({
            icon: "success",
            text: "Waiting Charge Successfully Updated",
          })
    ...
    >
      <Card sx={{ width: { lg: 500, md: 500, sm: 400, sx: 300 }, mt: "8vh" }}>
        ...
        <Formik
          ...
          enableReinitialize
          initialValues={
            config?.waitingCharge?.freeTime
              ? {
                  freeTime: config?.waitingCharge?.freeTime,
                  perMinute: config?.waitingCharge?.perMinute,
                }
              : initialValues
          }
        >
          {({ isSubmitting, isValid }) => (
            <Form>
              <CardContent>
                {WaitingChargeSchema?.map((inputItem) => (
